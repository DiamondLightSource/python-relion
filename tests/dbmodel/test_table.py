...
def test_adding_second_new_row(fake_table, unique_value):
    ...
    assert fake_table._tab["unique_value"] == [unique_value, unique_value + 1]
...
def test_multiple_counters_specified(fake_table_multiple_count, unique_value):
    assert fake_table_multiple_count._counters == ["count", "count02"]
    fake_table_multiple_count.add_row(
        {"unique_value": unique_value, "comment": "first insert"}
    )
    assert fake_table_multiple_count._tab["count"] == [1]
    assert fake_table_multiple_count._tab["count02"] == [1]
    fake_table_multiple_count.add_row(
        {"unique_value": unique_value + 1, "comment": "second insert"}
    )
    assert fake_table_multiple_count._tab["count"] == [1, 2]
    assert fake_table_multiple_count._tab["count02"] == [1, 2]


def test_single_required(fake_table_required, unique_value):
    assert fake_table_required._required == ["comment"]
    row = fake_table_required.add_row({"unique_value": unique_value})
    assert row is None
    assert fake_table_required._tab["unique_value"] == []
    row = fake_table_required.add_row(
        {"unique_value": unique_value, "comment": "first success"}
    )
    assert fake_table_required._tab["unique_value"] == [unique_value]


def test_multiple_required(fake_table_multiple_required, unique_value):
    assert fake_table_multiple_required._required == ["comment", "appendable"]
    row = fake_table_multiple_required.add_row({"unique_value": unique_value})
    assert row is None
    assert fake_table_multiple_required._tab["unique_value"] == []
    row = fake_table_multiple_required.add_row(
        {"unique_value": unique_value, "comment": "first attempt"}
    )
    assert fake_table_multiple_required._tab["unique_value"] == []
    row = fake_table_multiple_required.add_row(
        {"unique_value": unique_value, "comment": "first success", "appendable": 1}
    )
    assert fake_table_multiple_required._tab["unique_value"] == [unique_value]
...
def test_appending(fake_table, unique_value):
    ...
    pid_insert = fake_table.add_row(
        {"unique_value": unique_value, "comment": "first insert", "appendable": 5}
    )
    for columns in fake_table._tab.values():
        assert len(columns) == 1
    assert fake_table._tab["primary_id"] == [base_id + 1]
...
def test_appending_list_and_overlapping_list(fake_table, unique_value):
    ...
    assert fake_table._tab["appendable"] == [4]
    pid_insert = fake_table.add_row(
        {"unique_value": unique_value, "comment": "first insert", "appendable": [5, 6]}
    )
    for columns in fake_table._tab.values():
        assert len(columns) == 1
    assert fake_table._tab["primary_id"] == [base_id + 1]
...
def test_adding_rows_for_table_with_two_unique_columns(
    fake_double_unique_table, unique_value
):
    fake_double_unique_table.add_row(
        {"unique_value_01": unique_value, "unique_value_02": unique_value}
    )
    for columns in fake_double_unique_table._tab.values():
        assert len(columns) == 1
    fake_double_unique_table.add_row(
        {"unique_value_01": unique_value, "unique_value_02": unique_value + 1}
    )
    for columns in fake_double_unique_table._tab.values():
        assert len(columns) == 2
    fake_double_unique_table.add_row(
        {"unique_value_01": unique_value + 1, "unique_value_02": unique_value}
    )
    for columns in fake_double_unique_table._tab.values():
...
def test_get_row_index_for_single_index(fake_table, unique_value):
    ...
    index = fake_table.get_row_index("primary_id", base_id + 1)
    assert index == 0
    fake_table.add_row(
        {"unique_value": unique_value + 1, "comment": "second insert", "appendable": 4}
    )
    index = fake_table.get_row_index("primary_id", base_id + 2)
    assert index == 1
    index = fake_table.get_row_index("unique_value", unique_value)
    assert index == 0


def test_get_row_index_multiple_indices(fake_table, unique_value):
    fake_table.add_row(
        {"unique_value": unique_value, "comment": "first insert", "appendable": 4}
    )
    fake_table.add_row(
...
def test_get_row_index_not_present(fake_table, unique_value):
    fake_table.add_row(
        {"unique_value": unique_value, "comment": "first insert", "appendable": 4}
    )
    index = fake_table.get_row_index("unique_value", unique_value + 1)
...
def test_get_row_by_primary_key(fake_table, unique_value):
    ...
    row = fake_table.get_row_by_primary_key(base_id + 1)
    assert row["unique_value"] == unique_value
