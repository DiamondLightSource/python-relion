...
parser.add_argument(
    "-e", "--extended", action="store_true", help="Output extended information"
)
...
def metadata_mrc(doc, labels):
    ...
    if metadata["Direct Detector Electron Counting"]:
        lin_mod = "In counting mode"
    else:
        lin_mod = "In linear mode"
...
def camera_type(size):
    if size == K2_dim:
        detect = "K2"
        res = ""
    elif size == K2_dim_super:
        detect = "K2"
        res = "Super Resolution"
    elif size == K3_dim:
        detect = "K3"
        res = ""
    elif size == K3_dim_super:
        detect = "K3"
        res = "Super Resolution"
    elif size == Fal_dim:
        detect = "Falcon III"
        res = ""
    elif size == Fal_dim_super:
        detect = "Falcon III"
        res = "Super Resolution"
    else:
...
try:
    ...
    print(
        "No input file given. Please run as 'ccpem-python ex_header.py -i FILE_NAME'."
    )
    sys.exit()
